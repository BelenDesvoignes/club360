...
def has_active_suspension(
    ...
    return (
        get_active_suspension(db, user_id, reason, activity_id=activity_id) is not None
    )
...
def create_free_class_booking(db: Session, user_id: int, instance_id: int) -> Booking:
    ...
    if has_active_suspension(db, user_id, SUSPENSION_CLASE_LIBRE):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Tu cuenta está suspendida para reservar clases libres. Debés abonar la deuda pendiente en la sección de suspensiones para poder realizar nuevas reservas.",
        )

    return create_booking(
        db, user_id=user_id, instance_id=instance_id, subscription_id=None
    )
...
def cancel_booking(db: Session, booking_id: int, user_id: int) -> Booking:
    ...
    WaitingListService.process_waiting_list_on_cancellation(
        db, instance_id=booking.instance_id
    )
...
def create_booking_with_credit(
    ...
    if not has_instance_capacity(db, instance_id):
        raise HTTPException(
            status_code=400, detail="No hay cupos disponibles para esta clase."
        )
